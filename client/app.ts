...
export class App {
  ...
  private async handleGranularityChange() {
    ...
    const oldGranularityMs = granularityToMs(
      this.state.priceHistory?.getGranularity() || "ONE_HOUR",
    );
    ...
    console.log(
      "Old granularity:",
      this.state.priceHistory?.getGranularity(),
      "ms:",
      oldGranularityMs,
    );
    console.log(
    ...
    const lastPossibleCandleTime =
      Math.floor(currentEnd / oldGranularityMs) * oldGranularityMs;
    ...
    const isLastCandleVisible =
      lastPossibleCandleTime >= currentStart &&
      lastPossibleCandleTime <= currentEnd;
    ...
    if (isLastCandleVisible) {
      lastCandleX =
        ((lastPossibleCandleTime - currentStart) / currentTimeRange) *
        (canvasWidth / dpr);
      ...
      console.log(
        "Last candle not visible in viewport - using center preservation",
      );
    }

    console.log(
      "Last possible candle time:",
      new Date(lastPossibleCandleTime).toISOString(),
    );
    console.log("Current time:", new Date(now).toISOString());
    console.log("Hours from now:", hoursFromNow.toFixed(1));
    console.log(
      "Is viewing shifted data (past or future):",
      isViewingShiftedData,
    );
    ...
    if (isNearRightEdge) {
      ...
      console.log(
        "Using edge preservation mode - keeping rightmost candle at",
      ...
      for (
        let t = Math.floor(currentStart / oldGranularityMs) * oldGranularityMs;
        t <= currentEnd;
        t += oldGranularityMs
      ) {
        if (t >= currentStart && t <= currentEnd) {
          const x =
            ((t - currentStart) / currentTimeRange) * (canvasWidth / dpr);
      ...
      if (closestCandle) {
        ...
        console.log(
          "Using center preservation mode - keeping candle at",
          targetScreenPosition,
          "px",
        );
      } else {
    ...
    let targetCandleTime =
      Math.floor(referenceTime / newGranularityMs) * newGranularityMs;

    console.log(
      "Target candle in new granularity:",
      new Date(targetCandleTime).toISOString(),
    );
    ...
    const verifyX =
      ((targetCandleTime - newStart) / newTimeSpan) * (canvasWidth / dpr);
    console.log(
      "Verification - new X position will be:",
      verifyX,
      "px from left",
    );
    console.log(
      "Verification - difference from target:",
      verifyX - targetScreenPosition,
      "px",
    );
