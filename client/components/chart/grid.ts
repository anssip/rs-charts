...
export class HairlineGrid implements Drawable {
  public draw(context: DrawingContext): void {
    ...
    if (gridStyle === GridStyle.PercentageOscillator) {
      // For percentage oscillators (RSI, Stochastic, etc.), use the provided configuration
      // or fall back to default RSI settings
      const levels = oscillatorConfig?.levels || [0, 30, 50, 70, 100];
      const thresholds = oscillatorConfig?.thresholds || [30, 70];
      ...
      for (const level of levels) {
        ...
        if (y >= 0 && y <= canvas.height / dpr) {
          // Use different styles for threshold levels (overbought/oversold)
